...
directly — no cosine path → no embedder call during search. Seeding uses real_embedder
(local MiniLM HTTP service at localhost:8080, 384-dim).
"""
import pytest
...
@pytest.mark.asyncio
async def test_01_bfs_explicit_origin_returns_direct_neighbors(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_02_bfs_depth_3_traverses_full_chain(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_03_bfs_depth_1_excludes_far_nodes(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_04_bfs_empty_origin_returns_empty(bfs_driver, real_embedder):
    """#4: empty origin list → empty results, no error."""
    await seed_bfs_graph(bfs_driver, real_embedder, TEST_GROUP, TEST_GROUP_2)
...
@pytest.mark.asyncio
async def test_05_bfs_none_origin_returns_empty(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_06_bfs_depth_0_raises_value_error(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_07_bfs_depth_6_raises_value_error(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_08_bfs_directed_no_outgoing_returns_empty(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_09_bfs_directed_reverse_traversal_excluded(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_10_bfs_respects_group_filter_walk_only(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_11_bfs_honors_edge_types_filter(bfs_driver, real_embedder):
...
@pytest.mark.asyncio
async def test_12_bfs_node_search_excludes_origin(bfs_driver, real_embedder):
